...
if __name__ == "__main__":
    dataset_dict = {"GIST": 55}
    dataset_list = list(dataset_dict.keys())

    target_model = "EEGNet"
    model_list = ["DeepConvNet", "EEGInception", "PLNet", "PPNN"]
    ...
    for mode in train_mode:
        ...
        with open(result_txt_path, "w", encoding="utf-8"):
            pass

        for dataset in dataset_list:
            result_excel_path = result_dir / f"wilcoxon_{mode}_{dataset}_result.xlsx"
            result_data = []
                ...
                for model in model_list:
                    ...
                    try:
                        ...
                        with open(result_txt_path, "a", encoding="utf-8") as f:
                            f.write(log_line + "\n")
                        result_data.append(
                            [dataset, target_model, model, auc_stat, auc_p, ba_stat, ba_p, f1_stat, f1_p]
                        )
            ...
            if result_data:
                ...
                print(f"\n[Done] {dataset} results saved to: {result_excel_path}")
