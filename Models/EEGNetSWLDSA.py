...
class SimilarityWeightedDualStatisticsBatchNorm2d(nn.Module):
    def __init__(
        self,
        num_features: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            if use_target_stats:
                ...
                source_prior = self._weighted_source_prior(target_mean.detach(), target_var.detach())
        ...
        if use_target_stats and bool(self.target_initialized.item()):
            source_prior = self._weighted_source_prior(self.target_running_mean, self.target_running_var)
            source_mean = source_prior[0] if source_prior is not None else None
            source_var = source_prior[1] if source_prior is not None else None
...
class Model(nn.Module):
    def __init__(self, config: dict):
        ...
        self.target_blend_alpha = float(config.get("swldsa_target_blend_alpha", 0.1))
        self.similarity_tau = float(config.get("swldsa_similarity_tau", 1.0))
        self.var_distance_weight = float(config.get("swldsa_var_distance_weight", 1.0))
        ...
        self.block2_swldsa = SimilarityWeightedDualStatisticsBatchNorm2d(
            self.F2,
    ...
    def set_domain_context(self, domain_ids: torch.Tensor | None = None, *, use_target_stats: bool = False) -> None:
        self.block2_swldsa.set_domain_context(domain_ids=domain_ids, use_target_stats=use_target_stats)

    def _forward_features(self, x: torch.Tensor) -> torch.Tensor:
        x = self.block1(x)
        x = self.block2_conv(x)
        x = self.block2_swldsa(x)
...
if __name__ == "__main__":
    ...
    summarize_model("EEGNetSWLDSA", Model)
