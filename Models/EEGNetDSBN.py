    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            if use_target_stats:
                ...
                self._update_target_stats(mean, var)
        ...
        if use_target_stats and bool(self.target_initialized.item()):
            return self._normalize_with_stats(x, self.target_running_mean.to(x.dtype), self.target_running_var.to(x.dtype))
...
class Model(nn.Module):
    def __init__(self, config: dict):
        ...
        self.block2_dsbn = DomainSpecificBatchNorm2d(self.F2, num_source_domains=self.num_source_domains, eps=1.0e-5, momentum=0.1, affine=True)
    ...
    def set_domain_context(self, domain_ids: torch.Tensor | None = None, *, use_target_stats: bool = False) -> None:
        self.block2_dsbn.set_domain_context(domain_ids=domain_ids, use_target_stats=use_target_stats)

    def _forward_features(self, x: torch.Tensor) -> torch.Tensor:
        x = self.block1(x)
        x = self.block2_conv(x)
        x = self.block2_dsbn(x)
...
if __name__ == "__main__":
    ...
    summarize_model("EEGNetDSBN", Model)
