...
class DualStatisticsBatchNorm2d(nn.Module):
    def __init__(
        self,
        num_features: int,
        ...
        self.register_buffer("last_source_mix_mean", torch.zeros(self.num_features))
        self.register_buffer("last_source_mix_var", torch.ones(self.num_features))
        self.register_buffer("last_source_mix_initialized", torch.tensor(False, dtype=torch.bool))
    ...
    def _update_last_source_mix(self, mean: torch.Tensor, var: torch.Tensor) -> None:
        with torch.no_grad():
            self.last_source_mix_mean.copy_(mean.detach())
            self.last_source_mix_var.copy_(var.detach())
            self.last_source_mix_initialized.fill_(True)

    def _get_source_prior(self) -> tuple[torch.Tensor, torch.Tensor] | None:
        if bool(self.last_source_mix_initialized.item()):
            return self.last_source_mix_mean, self.last_source_mix_var
        return self._aggregate_source_stats()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            if use_target_stats:
                ...
                source_prior = self._get_source_prior()
            ...
            if isinstance(domain_ids, torch.Tensor) and domain_ids.numel() == x.shape[0]:
                ...
                for domain_id in torch.unique(domain_ids.detach()).tolist():
                    ...
                    batch_means.append(mean.detach())
                    batch_vars.append(var.detach())
                    batch_counts.append(int(mask.sum().item()))
                    with torch.no_grad():
                ...
                if batch_means:
                    total = float(sum(batch_counts))
                    mix_mean = sum((count / total) * mean for mean, count in zip(batch_means, batch_counts))
                    mix_var = sum((count / total) * var for var, count in zip(batch_vars, batch_counts))
                    self._update_last_source_mix(mix_mean, mix_var)

                if bool(covered.all().item()):
                    return out

            mean, var = self._batch_stats(x.float())
            self._update_last_source_mix(mean.detach(), var.detach())
            return self._normalize_with_stats(x, mean.to(x.dtype), var.to(x.dtype))
...
class Model(nn.Module):
    def __init__(self, config: dict):
        ...
        self.target_blend_alpha = float(config.get("ldsa_target_blend_alpha", 0.7))
        ...
        self.block2_ldsa = DualStatisticsBatchNorm2d(
            self.F2,
    ...
    def set_domain_context(self, domain_ids: torch.Tensor | None = None, *, use_target_stats: bool = False) -> None:
        self.block2_ldsa.set_domain_context(domain_ids=domain_ids, use_target_stats=use_target_stats)

    def _forward_features(self, x: torch.Tensor) -> torch.Tensor:
        x = self.block1(x)
        x = self.block2_conv(x)
        x = self.block2_ldsa(x)
...
if __name__ == "__main__":
    ...
    summarize_model("EEGNetLDSA", Model)
