...
class DiscriminabilityGuidedDualStatisticsBatchNorm2d(nn.Module):
    def __init__(
        self,
        num_features: int,
        ...
        self.target_anchor_domain_id: int | None = None
    ...
    def set_target_anchor_domain_id(self, domain_id: int | None) -> None:
        if domain_id in (None, 0):
            self.target_anchor_domain_id = None
        else:
            self.target_anchor_domain_id = int(domain_id)
    ...
    def _anchor_source_prior(self) -> tuple[torch.Tensor, torch.Tensor] | None:
        if self.target_anchor_domain_id is None:
            return None
        idx = self._domain_index(int(self.target_anchor_domain_id))
        if idx is None or not bool(self.source_initialized[idx].item()):
            return None
        return self.source_running_mean[idx], self.source_running_var[idx]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            if use_target_stats:
                ...
                source_prior = self._anchor_source_prior()
...
class Model(nn.Module):
    def __init__(self, config: dict):
        ...
        self.target_blend_alpha = float(config.get("dgldsa_target_blend_alpha", 0.1))
        ...
        self.block2_dgldsa = DiscriminabilityGuidedDualStatisticsBatchNorm2d(
            self.F2,
    ...
    def set_domain_context(self, domain_ids: torch.Tensor | None = None, *, use_target_stats: bool = False) -> None:
        self.block2_dgldsa.set_domain_context(domain_ids=domain_ids, use_target_stats=use_target_stats)

    def set_target_anchor_domain_id(self, domain_id: int | None) -> None:
        self.block2_dgldsa.set_target_anchor_domain_id(domain_id)

    def _forward_features(self, x: torch.Tensor) -> torch.Tensor:
        x = self.block1(x)
        x = self.block2_conv(x)
        x = self.block2_dgldsa(x)
