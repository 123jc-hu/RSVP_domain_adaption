...
class TangentSpaceHead(nn.Module):
    def __init__(
        ...
        self.classifier = DenseWithConstraint(self.out_dim, self.n_classes, bias=False, max_norm=0.25)
...
class Model(nn.Module):
    def __init__(self, config: dict):
        ...
        self.fusion_flat_weight = float(config.get("eegnet_dual_fusion_flat_weight", 0.7))
        self.fusion_flat_weight = max(0.0, min(1.0, self.fusion_flat_weight))
        self.fusion_ts_weight = 1.0 - self.fusion_flat_weight
        self.output_mode = str(config.get("eegnet_dual_output_mode", "fusion")).strip().lower()
    ...
    def forward(self, x, train_stage: int = 2, return_features: bool = False):
        ...
        flat_logits = self.flat_classifier(flat_features)
        ts_logits, ts_features = self.ts_head(feature_map)
        if self.output_mode == "flat":
            logits = flat_logits
        else:
            logits = self.fusion_flat_weight * flat_logits + self.fusion_ts_weight * ts_logits
        if return_features:
            return logits, {
                ...
                "ts_logits": ts_logits,
                "fusion_flat_weight": self.fusion_flat_weight,
                "fusion_ts_weight": self.fusion_ts_weight,
...
if __name__ == "__main__":
    ...
    summarize_model("EEGNetDualHead", Model)
